...
export class ValidationUtils {
	...
	static validateDrawing(
		...
		try {
			...
			let overlapCheck: {
				hasOverlap: boolean;
				overlappingLandmarkName?: string;
				overlappingLandmarkId?: string;
			} = { hasOverlap: false };
			if (extent && landmarks) {
				overlapCheck = this.checkOverlap(extent, landmarks, excludeLandmarkId, wktFormat);
			}
	...
	static validateBusStopLocation(
		...
		const selectedLandmark = landmarks.find((lm) => lm && lm.id === selectedLandmarkId);
...
export class FeatureUtils {
	 ...
	 */
	static removeFeatureFromSource(
		feature: Feature | null,
		source: VectorSource<Feature<Geometry>> | { removeFeature?: (feature: Feature) => void } | null
	): void {
		...
		return () => {
			try {
				...
				if (
					...
					validation.overlappingLandmarkId
				) {
					const rectInfo = GeometryUtils.circleToRectangle(geometry as CircleGeom);
