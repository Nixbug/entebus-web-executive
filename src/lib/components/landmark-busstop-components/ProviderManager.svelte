<script lang="ts">
	...
	function handleAddProvider() {
		...
		if (!newProviderUrl.includes('{x}') || !newProviderUrl.includes('{y}') || !newProviderUrl.includes('{z}')) {
	...
	function handleFileImport(event: Event) {
		...
		reader.onload = () => {
			...
			if (result.added > 0) {
				alert(`Imported ${result.added} provider(s)` + (result.skipped > 0 ? ` (${result.skipped} skipped)` : ''));
...
{#if show}
	...
	<div class="provider-modal">
			...
			{#each providers as provider}
				<div class="provider-item">
					<div class="provider-info">
						...
						{/if}
						{#if provider.url}
							<small class="provider-url" title={provider.url}>{provider.url.slice(0, 40)}...</small>
						{:else}
		...
		<div class="provider-actions">
			<button class="btn btn-sm btn-primary" on:click={() => (showAddProviderForm = !showAddProviderForm)}>
		...
		{#if showAddProviderForm}
			<div class="add-provider-form">
				...
				<div class="form-actions">
					...
					<button class="btn btn-sm" on:click={() => { showAddProviderForm = false; resetForm(); }}>Cancel</button>
				</div>
			</div>
		{/if}
		</div>
